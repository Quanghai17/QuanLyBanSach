  ...
  <div class="col-md-12">
    ...
    <div class="box box-primary">
      ...
      <tr>
          <td> Logo Footer
              <span class="input-group-btn">
                 <a data-input="images" data-preview="preview_image_footer" data-type="logo" class="btn btn-sm btn-flat btn-primary lfm">
                   <i class="fa fa-picture-o"></i> {{trans('product.admin.choose_image')}}
                 </a>
               </span>
          </td>
          <td>
              <div class="input-group">
                  <input type="hidden" id="images" name="logofooter" value="{{ $infos->logofooter }}" class="form-control input-sm image" placeholder=""  />
              </div>
              @if ($errors->has('logofooter'))
                  <span class="help-block">
                      {{ $errors->first('logofooter') }}
                  </span>
              @endif
              <div id="preview_image_footer" class="img_holder">{!! sc_image_render($infos->logofooter,'100px', '', 'logofooter') !!}</div>
      ...
      <tr>
          <td>Facebook</td>
          <td><a href="#" class="fied-required editable editable-click" data-name="facebook" data-type="text" data-pk="" data-source="" data-url="{{ route('admin_store_info.update') }}" data-title="Facebook" data-value="{{ $infos->facebook }}" data-original-title="" title="">{{$infos->facebook }}</a></td>
      </tr>
      <tr>
          <td>Instagram</td>
          <td><a href="#" class="fied-required editable editable-click" data-name="instagram" data-type="text" data-pk="" data-source="" data-url="{{ route('admin_store_info.update') }}" data-title="Instagram" data-value="{{ $infos->instagram }}" data-original-title="" title="">{{$infos->instagram }}</a></td>
      </tr>
      <tr>
          <td>Youtube</td>
          <td><a href="#" class="fied-required editable editable-click" data-name="youtube" data-type="text" data-pk="" data-source="" data-url="{{ route('admin_store_info.update') }}" data-title="Youtube" data-value="{{ $infos->youtube }}" data-original-title="" title="">{{$infos->youtube }}</a></td>
      </tr>
...
//End logo
  ...
  //Logo
  $('[name="logofooter"]').change(function(event) {
      $.ajax({
          url: '{{ route('admin_store_info.update') }}',
          type: 'POST',
          dataType: 'JSON',
          data: {"name": 'logofooter',"value":$(this).val(),"_token": "{{ csrf_token() }}",},
      })
          .done(function(data) {
              if(data.error == 0){
                  alertJs('success', '{{ trans('admin.msg_change_success') }}');
