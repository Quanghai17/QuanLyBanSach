...
<form action="{{ route('postRegister') }}" method="post" class="box">
    {!! csrf_field() !!}
    <div style="padding-bottom: 30px" style=" border: 2px solid rgb(5, 5, 5);
    border-radius: 25px;" class="form_content {{ old('check_red') ? 'in' : '' }}" id="collapseExample">
        ...
        @if (sc_config('customer_address2'))
            <div class="form-group{{ $errors->has('reg_address1') ? ' has-error' : '' }}">
                <input type="text"
                    ...
                    name="reg_address1" placeholder="{{ trans('account.address1') }}"
            ...
            <div class="form-group{{ $errors->has('reg_address2') ? ' has-error' : '' }}">
                <input type="text"
                    ...
                    name="reg_address2" placeholder="{{ trans('account.address2') }}"
                    value="{{ old('reg_address2') }}">
        ...
        @if (sc_config('customer_company'))
            <div class="form-group{{ $errors->has('reg_company') ? ' has-error' : '' }}">
                <input type="text"
                    ...
                    name="reg_company" placeholder="{{ trans('account.company') }}"
                    value="{{ old('reg_company') }}">
            ...
            <div class="form-group  {{ $errors->has('reg_country') ? ' has-error' : '' }}">
                <select class="form-control reg_country" style="width: 100%;" name="reg_country">
                    ...
                    @foreach ($countries as $k => $v)
                        <option value="{{ $k }}" {{ old('reg_country') == $k ? 'selected' : '' }}>
                            {{ $v }}</option>
                    @endforeach
                </select>
        ...
        <div class="submit">
            <button style="background-color: rgb(103, 201, 5)" type="submit" name="SubmitCreate"
                class="btn btn-default">{{ trans('account.signup') }}</button>
        </div>
    </div>
